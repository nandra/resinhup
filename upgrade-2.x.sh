...
function pre_update_fix_bootfiles_hook {
    log "Applying bootfiles hostapp-hook fix (local source)"
    local bootfiles_temp
    bootfiles_temp=$(mktemp)
    cat <<EOF > ${bootfiles_temp}
#!/bin/sh
...
# Variables
container_id="\$1"
bootfiles_list="/usr/share/hostapp-update/bootfiles.txt"
boot_fingerprint="resinos.fingerprint"
bootfiles_blacklist="\\
	/config.json \\
	/config.txt \\
...
# Checks if a file is present in the ignore list, not to extract
# Arguments:
# 	\$1: file to be checked
# Return value:
# 	0: file is ignored
...
deploy() {
	...
	if isIgnored "\$_file"; then
		printf " file is shipped but will ignore.\n"
	elif isModified "\$_file"; then
